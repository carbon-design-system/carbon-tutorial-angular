...
function generateFiles(host, srcFolder, target, substitutions) {
    ...
    allFilesInDir(srcFolder).forEach((filePath) => {
        ...
        if (binaryExts.has(path.extname(filePath))) {
            newContent = fs_1.readFileSync(filePath);
        }
        else {
            const template = fs_1.readFileSync(filePath, 'utf-8');
            try {
                newContent = ejs.render(template, substitutions, {});
            }
            catch (e) {
                logger_1.logger.error(`Error in ${filePath.replace(`${host.root}/`, '')}:`);
                throw e;
            }
        }
...
function allFilesInDir(parent) {
    let res = [];
    try {
        fs_1.readdirSync(parent).forEach((c) => {
