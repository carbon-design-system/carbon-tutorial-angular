...
class FsTree {
    ...
    delete(filePath) {
        ...
        if (this.children(path_1.dirname(this.rp(filePath))).length < 1) {
    ...
    fsReadDir(dirPath) {
        try {
            return fs_extra_1.readdirSync(path_1.join(this.root, dirPath));
        }
        catch (_a) {
            return [];
        }
    ...
    fsIsFile(filePath) {
        const stat = fs_extra_1.statSync(path_1.join(this.root, filePath));
        return stat.isFile();
    }
    fsReadFile(filePath) {
        return fs_extra_1.readFileSync(path_1.join(this.root, filePath));
    }
    fsExists(filePath) {
        try {
            const stat = fs_extra_1.statSync(path_1.join(this.root, filePath));
...
function flushChanges(root, fileChanges) {
    fileChanges.forEach((f) => {
        ...
        if (f.type === 'CREATE') {
            fs_extra_1.ensureDirSync(path_1.dirname(fpath));
            fs_extra_1.writeFileSync(fpath, f.content);
            if ((_a = f.options) === null || _a === void 0 ? void 0 : _a.mode)
                fs_extra_1.chmodSync(fpath, f.options.mode);
        }
