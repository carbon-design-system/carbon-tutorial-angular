...
import { Directive, Input, Output, EventEmitter, ElementRef, ViewContainerRef, HostBinding, Optional } from "@angular/core";
...
var DialogDirective = /** @class */ (function () {
    ...
    eventService) {
        if (eventService === void 0) { eventService = null; }
     ...
     */
    DialogDirective.prototype.onTouchStart = function (event) {
    ...
    DialogDirective.prototype.ngOnInit = function () {
        ...
        if (this.eventService) {
            ...
            fromEvent(element, "keydown").subscribe(function (event) {
                // "Esc" is an IE specific value
                if (event.target === _this.dialogConfig.parentRef.nativeElement &&
                    (event.key === "Tab" || event.key === "Tab" && event.shiftKey) ||
                    event.key === "Escape" || event.key === "Esc") {
            ...
            // bind events for hovering or clicking the host
            if (this.trigger === "hover" || this.trigger === "mouseenter") {
                fromEvent(element, "mouseenter").subscribe(function () { return _this.open(); });
                fromEvent(element, this.closeTrigger).subscribe(function () { return _this.close(); });
                fromEvent(element, "focus").subscribe(function () { return _this.open(); });
                fromEvent(element, "blur").subscribe(function () { return _this.close(); });
            }
            else {
                fromEvent(element, "click").subscribe(function () { return _this.toggle(); });
                fromEvent(element, "keydown").subscribe(function (event) {
                    // "Spacebar" is an IE specific value
                    if (event.key === "Enter" || event.key === " " || event.key === "Spacebar") {
                        setTimeout(function () {
